...
const getCorsOrigin = async (): Promise<string[]> => {
  try {
    ...
    if (value === undefined) {
      ...
      // value = cors?.origin || [];
      value = [ config.client_side_url as string, 'http://localhost:5173', 'http://localhost:5174']
    ...
    return [ config.client_side_url as string, 'http://localhost:5173', 'http://localhost:5174'] // Fallback origins
  }
}
